...
- **O provedor falso é recusado em produção** (`ENVIRONMENT=production`): ele "autorizaria" notas que não existem.
...
1. **Provedor falso em produção** emitindo notas inexistentes. → `fake_so_em_desenvolvimento` (Task 3) e `provedor_falso_em_producao_conta_como_nao_configurado...` (Task 6).
...
4. **Nota autorizada nunca regride** e **repetir a emissão não duplica**; pedido com nota autorizada não pode ser cancelado enquanto o cancelamento fiscal não existe. → `skip_nunca_desfaz...`, `tentar_de_novo_apos_falha...` (Task 5), `pedido_com_nfce_autorizada_nao_pode_ser_cancelado_ainda` (Task 6).
...
```
Expected: `32 passed` (17 já existentes + 15 novos).
...
cd backend && python -m pytest -q; cd ..
```
Expected: `69 passed` (57 + 12). Os avisos de `DeprecationWarning` do Alembic (`path_separator`) são esperados.
...
- Produces (rotas, todas sob `/api/v1`):
...
EDITS = {
  ...
  "backend/app/services/order_service.py": [
    [
      "Não cancelar pedido com NFC-e autorizada (até o C3)",

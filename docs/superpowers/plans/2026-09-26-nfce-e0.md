...
- **Cofre:** AES-256-GCM, nonce aleatório de 12 bytes na frente do texto cifrado, e o **tipo do segredo vai como dado autenticado** (`certificate`, `certificate_password`, `csc_hml`, `csc_prod`): um texto cifrado copiado para outro tipo **não abre**. A chave mestra é `FISCAL_SECRET_KEY` (32 bytes em base64), lida **só** do ambiente (Coolify). Sem ela ou com ela inválida o cofre fica **indisponível**: a API recusa guardar (não guarda em claro) e a tela avisa. Trocar ou perder a chave invalida o que foi guardado; documentado.
...
- **Trava de produção:** só passa com modo real (`REAL_MODES`) **e** disponível, emissão ligada com os dados da empresa, certificado do cofre válido e do mesmo CNPJ, CSC de produção cadastrado, ao menos uma venda de teste em homologação (`hml-`, autorizada **ou** cancelada) e a confirmação do contador. Como o adaptador direto só chega na E2, **produção continua inalcançável**: é o comportamento desejado, não um defeito.
...
8. **Frontend:** o secreto ficando em estado depois do envio, botão habilitado sem arquivo/senha, aviso do cofre ausente, `mode` sumindo do corpo do PUT, vírgula das alíquotas. → roteiros das Tasks 5 e 6.
...
- [ ] **Step 4: Entregar ao usuário e parar**

...
- **Dois desvios leves do cupom (registrados em `docs/fiscal/README.md`):** a hora de emissão é a do servidor ao receber a autorização (o layout oficial usa a do XML e prevê a data da autorização), e o CPF/CNPJ do consumidor vem do cadastro do cliente **de hoje** (a reimpressão pode diferir se o cadastro mudar). Resolvem-se com o adaptador do provedor real.
...
7. **A contagem de consultas terminando sem ninguém perceber:** o React Query só re-renderiza por propriedades **lidas**; com o pedido voltando igual, o `gaveUp` (e a impressão automática) nunca disparava. `useOrderFiscal` lê `dataUpdatedAt` de propósito. → roteiro do navegador da Task 4 (cenário "a nota nunca assenta": depois de 6 consultas, selo "NFC-e ainda em andamento" e uma impressão do cupom não fiscal).
...
3. **A nota nunca assenta:** responda `GET /orders/o1` sempre com o pedido `not_required` sem motivo. Depois de **6 consultas** o selo vira **"NFC-e ainda em andamento"** e sai **uma** impressão do **cupom não fiscal** (o caixa não fica sem cupom).
4. **Padrão (emissão desligada):** recarregue e responda `GET /fiscal/status` com `{enabled:false, provider_configured:false, environment:'homologacao', issuer:null}`. Confirme um pedido: a impressão automática dispara **na hora** (em menos de meio segundo) com **`CUPOM NÃO FISCAL`**, o botão diz **"Imprimir cupom"** e **nenhum selo** de NFC-e aparece.

Expected: os 4 comportamentos. Se algum divergir, corrija antes do commit. Desligue a impressão automática (`pdv-printer` com `enabled:false`) e pare o servidor de preview ao terminar.

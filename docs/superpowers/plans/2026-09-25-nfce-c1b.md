...
2. **A linha de status consultando para sempre** ou nunca parando. → `fiscalPollDelay` e `MAX_FISCAL_POLLS` (Task 1) e o roteiro da Task 4.
...
1. Aparece o aviso âmbar **"Ambiente de homologação: as notas emitidas não têm valor fiscal."**, o cartão "Empresa emitente" com os campos preenchidos (CNPJ, série, UF…) e a linha "1 produto ainda sem dados fiscais completos…".
2. **Salvar dados fiscais** faz `PUT /fiscal/settings` com o corpo certo (`series` como número, documentos só com dígitos, vazio como `null`) e mostra **"Dados fiscais salvos."**.

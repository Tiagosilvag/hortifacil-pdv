...
- **Reenvio automático só reenvia o que já foi ao provedor** (`pending` com `fiscal_attempts > 0`, últimas 24 h). Pendente por dado fiscal faltando espera alguém corrigir o cadastro. A emissão é idempotente pela referência, então repetir nunca duplica a nota. O laço **nunca levanta** e não roda sem provedor configurado nem com `FISCAL_RETRY_INTERVAL_SECONDS=0`.
...
- **Migration 013 aditiva e reversível** (colunas anuláveis ou com valor padrão). Não rodar contra produção; **sem deploy** sem o "pode fazer o deploy" explícito do usuário.
...
6. **Frontend:** motivo curto habilitando o botão; erro do servidor sumindo; Enter no campo "PRODUÇÃO" reenviando o formulário. → roteiros das Tasks 5 e 6.
...
| `models/order.py`, `models/fiscal.py`, `schemas/fiscal.py`, `schemas/order.py`, `core/config.py`, `.env.example`, `fiscal_service.py`, `order_service.py`, `api/v1/fiscal.py`, `main.py` (modificar) | Ligar tudo |
...
- Modify: `backend/app/services/fiscal_service.py`, `backend/app/services/order_service.py`, `backend/tests/test_fiscal_api.py`
...
Mesmo procedimento de edição: a emissão grava a referência usada no provedor e o cancelamento de pedido passa a cancelar a nota primeiro.
...
cd backend && python -m pytest -q; cd ..
```
Expected: `148 passed` (125 + 23).

- [ ] **Step 5: Commit**

```bash
git add backend/app/services/fiscal_cancel.py backend/app/services/fiscal_service.py backend/app/services/order_service.py backend/tests/test_fiscal_cancel.py backend/tests/test_fiscal_api.py
...
git status --short
```
Expected: backend `171 passed`; frontend `169 passed`, `tsc` limpo, `built in`; `git status` sem arquivos do C3 pendentes (só, no máximo, a alteração do usuário no `NewOrder.tsx` e `.impeccable/config.json`).
